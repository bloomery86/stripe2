    ...
    if (confirmPaymentBtn) {
        confirmPaymentBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // Fetch WhatsApp credentials from server
                try {
                    const credentialsResponse = await fetch('/api/whatsapp-credentials');
                    if (!credentialsResponse.ok) {
                        throw new Error('Failed to get WhatsApp credentials');
                    }
                    const credentials = await credentialsResponse.json();
                    
                    const whatsappUrl = `https://api.callmebot.com/whatsapp.php?phone=${credentials.phone}&text=${encodeURIComponent(message)}&apikey=${credentials.apiKey}`;
